...
public class Ruins1Feature extends AbstractTempleFeature<DefaultFeatureConfig> {

    public Ruins1Feature() {
    ...
    public StructureStartFactory getStructureStartFactory() {
        return Ruins1StructureStart::new;
    }

    public static class Ruins1StructureStart extends StructureStart {
        public Ruins1StructureStart(StructureFeature<?> structureFeature_1, int int_1, int int_2, BlockBox box, int int_3,
                long long_1) {
        ...
        public void initialize(ChunkGenerator<?> chunkGenerator, StructureManager structureManager, int x, int z,
                Biome biome) {
            DefaultFeatureConfig defaultFeatureConfig = (DefaultFeatureConfig)chunkGenerator.getStructureConfig(biome, FeatureReg.Ruin1Feature);
            int i = x * 16;
            int j = z * 16;
            BlockPos blockPos = new BlockPos(i, 0, j);
            BlockRotation blockRotation = BlockRotation.NONE;
            //.values()[this.random.nextInt(BlockRotation.values().length)];
            Ruins1Gen.addParts(structureManager, blockPos, blockRotation, this.children, this.random,
